...
- Fix `premium.types.CoreOverwriteError`
...
- Add `premium.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in premium inline
...
- Change inline query placeholder to `user@premium:~$` + legacy migration
...
- Create new type :obj:`premium.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`premium.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`premium.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Premium-TL >= 1.24.9
...
- Publish premium telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `PremiumSecurity`
- Fix `.uninstall_premium` being accessible by sudo
- Fix `utils.find_caller` for :method:`premium.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Premium emojis to premium users
...
- Move `premium._types` to `premium.types` with legacy support
- Remake all core modules to decorators
- Force custom premium telethon installation with 144 layer support
...
- Add support for `# scope: premium_min` metatag in libraries
...
- Update quickstart
- New fields in PremiumInfo
- Remove suffix `Premium` if user specified it somewhere below
...
- Fix processing of `# scope: premium_min`
...
- Add `banner_url` config var to `PremiumInfo`
...
- Add `client.force_get_entity` to bypass Premium Cacher
...
- Fix translation issue in PremiumConfig
- Fix `.dump`
...
- Add banner with Premium installation status
...
- Return :obj:`InlineMessage` in `premium.inline.gallery.Gallery.gallery`
...
- Add heroku waker
- Make `premium-` app naming optional
...
- Merge `premium_logger.py` with `test.py`
- Localization
...
- Replace .uninstall\_premium with full uninstallation (remove bot, asset chats and folder)
...
- Add automatic premium folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to premium folder)
...
- Introducing PremiumDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor PremiumSecurity, especially the bounding mask control
...
- Add ability to disable PremiumDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Premium 1.0.22

Add PremiumDL module to use inline download buttons in verified channels
