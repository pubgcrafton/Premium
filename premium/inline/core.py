...
class InlineManager(
    ...
    def __init__(
        ...
        self._token = db.get("premium.inline", "bot_token", False)
    ...
    async def _register_manager(
        ...
        self._name = get_display_name(self._client.premium_me)
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start premium init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start premium init"
                )
            except Exception:
