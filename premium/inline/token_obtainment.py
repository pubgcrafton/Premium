...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("premium.inline", "custom_bot", False):
                username = self._db.get("premium.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            for msg in [
                f"💢 Premium Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get(
                        "premium.inline", "custom_bot", False
                    ) and self._db.get(
                        "premium.inline", "custom_bot", False
                    ) != button.text.strip(
                        "@"
                    ...
                    self._db.set("premium.inline", "bot_token", token)
                    self._token = token
    ...
    async def _dp_revoke_token(self, already_initialised: bool = True):
        ...
        self._db.set("premium.inline", "bot_token", None)
        self._token = None
        if already_initialised:
